import React from "react";

const Eyebrows: React.FC<{ type: string }> = ({ type }) => {
  switch (type) {
    case "default":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M128 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M232 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "default2":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "raised":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "sad":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "sad2":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "unibrow":
      return (
        <path
          d="M125 158 Q160 152 195 158"
          stroke="#222"
          strokeWidth="3"
          fill="none"
        />
      );
    case "updown":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "updown2":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "angry":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    case "angry2":
      return (
        <g>
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M124 138c-1,1 -3,1 -4,0 -1,-1 -1,-2 0,-3 5,-6 10,-9 16,-11 6,-2 13,-2 20,0 2,0 3,2 2,3 0,2 -1,3 -3,2 -6,-1 -12,-2 -17,0 -5,1 -10,4 -14,9z" />
          <path fill="#000000" fillOpacity="0.7" fillRule="nonzero" d="M196 138c1,1 2,1 3,0 2,-1 2,-2 1,-3 -5,-6 -11,-9 -17,-11 -6,-2 -13,-2 -20,0 -1,0 -2,2 -2,3 1,2 2,3 3,2 7,-1 13,-2 18,0 5,1 10,4 14,9z" />
        </g>
      );
    default:
      return null;
  }
};

export default Eyebrows;
