...
import { calendarAnimations, variants, useCalendarAnimations } from '../animations/CalendarAnimations';
import { 
  format, 
  startOfMonth, 
  endOfMonth, 
...
const NeomorphicDayCell: React.FC<{
  ...
  isSelected: boolean;
  index: number;
}> = ({ date, dayData, isCurrentMonth, isToday, onClick, isSelected, index }) => {
  const [isHovered, setIsHovered] = useState(false);
  const { getNeomorphicHover } = useCalendarAnimations();
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${isSelected ? 'shadow-inner shadow-[#7a2323]/20' : 'shadow-[inset_2px_2px_6px_rgba(199,191,180,0.3),inset_-2px_-2px_6px_rgba(255,255,255,0.7)]'}
        ${isHovered ? 'shadow-[2px_2px_12px_rgba(122,35,35,0.15)]' : ''}
        aspect-square flex flex-col items-center justify-center p-1 min-h-[60px]
      `}
      variants={variants.calendarDay}
      initial="hidden"
      animate="visible"
    ...
    >
      ...
      <motion.div 
        ...
        `}
        animate={isToday ? calendarAnimations.dayCell.today : {}}
      >
        {format(date, 'd')}
      </motion.div>
        ...
        >
          {[...Array(Math.min(dayData.flowIntensity, 5))].map((_, i) => (
            <motion.div
              ...
              animate={calendarAnimations.flowDots.pulse}
              transition={{ 
                duration: 1, 
                repeat: Infinity, 
                delay: i * 0.1 
      ...
      {isToday && (
        <motion.div
          ...
          animate={calendarAnimations.special.glow}
        />
      )}
    </motion.div>
  );
...
export const NeomorphicCalendar: React.FC<NeomorphicCalendarProps> = ({ className = '' }) => {
  ...
  // Hook de animaciones personalizado
  const { createStagger, getEntranceAnimation, getNeomorphicHover } = useCalendarAnimations();
  ...
  const navigatePrevious = () => {
    const newDate = viewType === 'day' ? subDays(currentDate, 1) :
                   viewType === 'week' ? subWeeks(currentDate, 1) :
                   subMonths(currentDate, 1);
    setCurrentDate(startOfDay(newDate));
  };

  const navigateNext = () => {
    const newDate = viewType === 'day' ? addDays(currentDate, 1) :
                   viewType === 'week' ? addWeeks(currentDate, 1) :
  ...
  const handleDayClick = (date: Date) => {
    ...
    const dayData = calendarDays?.find(day => day.date === formattedDate) || null;
  ...
  if (isLoading) {
    return (
      <div className={`flex items-center justify-center h-[calc(100vh-200px)] ${className}`}>
        <motion.div
          animate={calendarAnimations.loading.spinner}
    ...
    <div className={`h-[calc(100vh-200px)] flex flex-col ${className}`}>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <motion.h2 
              ...
              key={getViewTitle()}
              {...calendarAnimations.navigation.title}
            >
              {getViewTitle()}
            </motion.h2>
            ...
            <div className="flex gap-2">
              <motion.button
                ...
                {...calendarAnimations.navigation.button}
              >
        ...
        >
          {Object.entries(phaseConfig).map(([phase, config], index) => (
            ...
            >
              ...
              />
              <span className="text-[#7a2323] capitalize">{phase.toLowerCase()}</span>
            </motion.div>
          ))}
        </motion.div>
      </motion.div>
      ...
      <div className="flex-1 min-h-0">
        <AnimatePresence mode="wait">
          {viewType === 'month' && (
            <motion.div
              key="month"
              {...calendarAnimations.viewTransition.month}
              className="h-full flex flex-col"
            >
              {/* Días de la semana */}
              <motion.div 
                className="grid grid-cols-7 gap-2 mb-4 flex-shrink-0"
                variants={variants.calendarGrid}
                initial="hidden"
                animate="visible"
              >
              ...
              >
                {viewDates.map((date, index) => {
                  const formattedDate = format(date, 'yyyy-MM-dd');
                  const dayData = safeCalendarDays.find(day => day.date === formattedDate);
                  const isCurrentMonth = isSameMonth(date, currentDate);
                  const isCurrentDay = isToday(date);
                  const isSelected = selectedDate && format(selectedDate, 'yyyy-MM-dd') === formattedDate;

                  return (
                    <div key={formattedDate} className="h-full">
                      <NeomorphicDayCell
                        date={date}
                        dayData={dayData}
          ...
          {viewType === 'week' && (
            <motion.div
              key="week"
              {...calendarAnimations.viewTransition.week}
              className="h-full"
            >
              <motion.div 
                  ...
                  return (
                    ...
                    >
                      <div className="text-center mb-3">
                        <motion.div 
                          className="text-xs text-[#7a2323] opacity-70"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 0.7 }}
                          transition={{ delay: index * 0.1 }}
                        >
                        ...
                        <motion.div 
                          className={`text-lg font-medium ${isCurrentDay ? 'text-[#7a2323] font-bold' : 'text-gray-900'}`}
                          animate={isCurrentDay ? calendarAnimations.dayCell.today : {}}
                        >
                          {format(date, 'd')}
                        </motion.div>
                      </div>
                      <motion.div 
                        className={`
                          ...
                          ${dayData?.phase ? `bg-gradient-to-br ${phaseConfig[dayData.phase].color}` : 'bg-[#e7e0d5]'}
                    ...
                    >
                      <div className="text-center mb-3">
                        <div className="text-xs text-[#7a2323] opacity-70">{format(date, 'EEE', { locale: es })}</div>
                        <div className={`text-lg font-medium ${isCurrentDay ? 'text-[#7a2323] font-bold' : 'text-gray-900'}`}>
                          {format(date, 'd')}
                        </div>
                      </div>
                      <div 
          ...
          {viewType === 'day' && (
            <motion.div
              key="day"
              {...calendarAnimations.viewTransition.day}
                      ...
                      <div className="p-8">
                        {dayData ? (
                          ...
                          >
                            {dayData.phase && (
                              ...
                              >
                                <motion.div 
                                  ...
                                  animate={calendarAnimations.special.float}
                                >
                                  {phaseConfig[dayData.phase].icon}
                                </motion.div>
                            ...
                            {dayData.flowIntensity && dayData.flowIntensity > 0 && (
                              <motion.div variants={variants.symptomItem}>
                                <h4 className="font-medium text-[#7a2323] mb-2">Intensidad del flujo</h4>
                                <div className="flex gap-2">
                                  {[...Array(5)].map((_, i) => (
                                    <motion.div 
                                      key={i} 
                                      className={`w-4 h-4 rounded-full ${
                                        i < dayData.flowIntensity! ? 'bg-red-500' : 'bg-gray-200'
                                      }`}
                                      initial={{ scale: 0 }}
                                      animate={{ scale: 1 }}
                                      transition={{ 
                            ...
                            {dayData.symptoms && dayData.symptoms.length > 0 && (
                              <motion.div variants={variants.symptomItem}>
                                <h4 className="font-medium text-[#7a2323] mb-2">Síntomas</h4>
                                <motion.div 
                            ...
                            {dayData.mood && dayData.mood.length > 0 && (
                              <motion.div variants={variants.symptomItem}>
                                <h4 className="font-medium text-[#7a2323] mb-2">Estado de ánimo</h4>
                                <motion.div 
                                  className="flex flex-wrap gap-2"
                                  variants={variants.symptomList}
                                  initial="hidden"
                          ...
                          <motion.div 
                            ...
                            {...calendarAnimations.fadeInScale}
                          >
                            <motion.div
                              animate={calendarAnimations.special.float}
                            >
                            ...
                            <p className="text-[#7a2323] opacity-70 mb-4">No hay información para este día</p>
                          </motion.div>
                        )}
      ...
      <AnimatePresence>
        {isModalOpen && selectedDate && (
          <motion.div
            {...calendarAnimations.modal.backdrop}
            className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4"
            onClick={() => setIsModalOpen(false)}
          >
            <motion.div
              {...calendarAnimations.modal.content}
            ...
            >
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <motion.div 
                    className="bg-[#e7e0d5] rounded-2xl p-4 shadow-[inset_2px_2px_6px_rgba(199,191,180,0.3),inset_-2px_-2px_6px_rgba(255,255,255,0.7)]"
              ...
              >
                ...
                <motion.button
                  onClick={async () => {
                    const flowSelect = document.getElementById('flowIntensity') as HTMLSelectElement;
