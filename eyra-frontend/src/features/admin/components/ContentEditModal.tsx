...
const ContentEditModal: React.FC<ContentEditModalProps> = ({
  ...
  useEffect(() => {
    if (content) {
      setFormData({
        ...
        targetPhase: content.targetPhase,
        tags: content.tags || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminContentService.updateContent(content.id.toString(), formData);
      onSave();
    } catch (err: any) {
      setError(err.message || 'Error al actualizar el contenido');
    } finally {
      setLoading(false);
    }
  };
